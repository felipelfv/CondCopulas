test_that("computationOf_G_chi_ZU returns box probabilities", {
  ...
  expect_equal(computationOf_G_chi_ZU(0.1, 0.1, 0.1)$G, array(c(1, rep(0, 124)), c(5, 5, 5)))

  # random data: must match direct box counts, and G_indep the product of marginals
  set.seed(123)
  Z <- matrix(runif(150), 50)
  for (nBoxes in c(2, 5, 7)) {
    res <- computationOf_G_chi_ZU(Z[, 1], Z[, 2], Z[, 3], nBoxes)
    G <- unclass(unname(table(lapply(1:3, function(j) cut(Z[, j], 0:nBoxes / nBoxes, include.lowest = TRUE))))) / 50
